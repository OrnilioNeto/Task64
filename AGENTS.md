...
- Triaging Sentry issues: invoke `sentry-triage`.
...
- After changing the API, run `mage generate:frontend-client` (generates `frontend/src/client/generated/` from the v2 OpenAPI spec) and commit the result. CI fails (`check:frontend-client`) when it's stale.
...
For frontend dev against a locally running API, set `DEV_PROXY` (e.g. `DEV_PROXY=http://localhost:3456`) — `vite.config.ts` then proxies `/api` to the backend.
...
`mage check:translations` verifies that every key used in code exists in `en.json` and flags dead keys — run it after adding or removing user-facing strings.
