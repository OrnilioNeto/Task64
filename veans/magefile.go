...
// Requires VEANS_E2E_API_URL to point at a running Task64 instance and
// either VEANS_E2E_TESTING_TOKEN (matching the API's TASK64_SERVICE_TESTINGTOKEN
...
func (Test) E2E() error {
	if os.Getenv("VEANS_E2E_API_URL") == "" {
		return fmt.Errorf("VEANS_E2E_API_URL is not set — start a Task64 instance and export the URL")
	}
	if os.Getenv("VEANS_E2E_ADMIN_TOKEN") == "" && os.Getenv("VEANS_E2E_TESTING_TOKEN") == "" {
		return fmt.Errorf("set VEANS_E2E_ADMIN_TOKEN, or VEANS_E2E_TESTING_TOKEN (matching the API's TASK64_SERVICE_TESTINGTOKEN) so the suite can seed its own admin")
	}
