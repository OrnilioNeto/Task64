...
func checkPostgresSchemaMismatch(x *xorm.Engine) error {
	...
	// users + migration together identify a schema holding an existing Task64 install.
	...
	if err != nil {
		return fmt.Errorf("could not check for existing Task64 tables: %w", err)
	}
...
func validateSchemaPlacement(configuredSchema, currentSchema string, dataSchemas []string) error {
	...
	if configuredSchema != "" && currentSchema != configuredSchema {
		return fmt.Errorf("the configured schema %q does not exist or is not accessible to the database user (active schema: %q). Create it or set database.schema (TASK64_DATABASE_SCHEMA) to an existing schema", configuredSchema, currentSchema)
	}
	...
	if len(dataSchemas) == 0 || found {
		if found && len(others) > 0 {
			log.Warningf("Found Task64 tables in schema(s) %s in addition to the active schema %q. Task64 will ignore them, but you may want to clean them up.", strings.Join(others, ", "), currentSchema)
		}
		return nil
	}

	if currentSchema == "" {
		return fmt.Errorf("the configured database schema does not exist, but existing Task64 tables were found in schema(s) %s. Set database.schema (TASK64_DATABASE_SCHEMA) to the schema containing your data", strings.Join(others, ", "))
	}

	return fmt.Errorf("existing Task64 tables were found in schema(s) %s, but Task64 is configured to use schema %q. Running migrations now would create a second, empty set of tables. Set database.schema (TASK64_DATABASE_SCHEMA) to the schema containing your data", strings.Join(others, ", "), currentSchema)
}
