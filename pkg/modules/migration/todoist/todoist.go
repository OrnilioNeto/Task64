...
// @Description Returns the auth url where the user needs to get its auth code. This code can then be used to migrate everything from todoist to Task64.
// @tags migration
// @Produce json
// @Security JWTKeyAuth
...
func convertTodoistToTask64(sync *sync, doneItems map[string]*doneItem) (fullTask64Hierachie []*models.ProjectWithTasksAndBuckets, err error) {
	...
	fullTask64Hierachie = append(fullTask64Hierachie, parent)

	// A map for all task64 lists with the project id they're coming from as key
	lists := make(map[string]*models.ProjectWithTasksAndBuckets, len(sync.Projects))

	// A map for all task64 tasks with the todoist task id as key to find them easily and add more data
	tasks := make(map[string]*models.TaskWithComments, len(sync.Items))

	// A map for all task64 labels with the todoist id as key to find them easier
...
// Migrate gets all tasks from todoist for a user and puts them into task64
// @Summary Migrate all lists, tasks etc. from todoist
// @Description Migrates all projects, tasks, notes, reminders, subtasks and files from todoist to task64.
// @tags migration
// @Accept json
// @Produce json
// @Security JWTKeyAuth
...
func (m *Migration) Migrate(u *user.User) (err error) {
	...
	fullTask64Hierachie, err := convertTodoistToTask64(syncResponse, doneItems)
	if err != nil {
		return
	}
	...
	err = migration.InsertFromStructure(fullTask64Hierachie, u)
	if err != nil {
		return
	}
