...
import (
	...
	"github.com/OrnilioNeto/Task64/pkg/routes"
...
func setupAutoTLS(server *http.Server) {
	...
	if config.ServiceInterface.GetString() != ":443" {
		log.Warningf("Task64's interface is set to %s, with tls it is recommended to set this to :443", config.ServiceInterface.GetString())
	}
...
var webCmd = &cobra.Command{
	...
	Run: func(_ *cobra.Command, _ []string) {
		...
		log.Infof("Task64 version %s", version.Version)
